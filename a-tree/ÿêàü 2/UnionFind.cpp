...
Team *UnionFind::Find(int id) {
    ...
    permutation_t sum_rS = permutation_t::neutral();
    int sum_plays = 0;
    while (node->parent != nullptr) {
        sum_rS = node->rS * sum_rS;
        sum_plays += node->temp_plays;
        node = node->parent;
    }
    UFNode *nodePlayer = Player.my_UFNode;
    while (nodePlayer != node) {
        permutation_t temp_rs = nodePlayer->rS;
        int temp_plays = nodePlayer->temp_plays;
        nodePlayer->rS =  sum_rS * permutation_t::neutral();
        nodePlayer->temp_plays = sum_plays;
        sum_rS = temp_rs.inv() * sum_rS;
        sum_plays -= temp_plays;
