...
vector<int> getRecordsStocks(ifstream& infile);

int main()
{
  string op;
  RecordsCompany *test_obj = new RecordsCompany();
  ifstream infile("./example_test.in");
  while (infile >> op)
  {
    ...
    {
      // get params
      vector<int> stocks_vec = getRecordsStocks(infile);
    ...
    else if(!op.compare("addPrize"))
    {
      ...
      infile >> c_id1; infile >> c_id2; infile >> amount;
